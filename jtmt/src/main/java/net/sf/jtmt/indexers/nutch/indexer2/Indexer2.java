...
public class Indexer2 extends Configured implements Tool {
  ...
  private static class Mapper2 extends Mapper<Text,LongWritable,Text,Text> {
    ...
    public void setup(Context context) {
      if (LABEL_CUTOFF_SCORE == null) {
        LABEL_CUTOFF_SCORE = 
          context.getConfiguration().getFloat("label.cutoff.score", 0.0F); 
      }
    }
    ...
    public void map(Text key, LongWritable value, Context context) 
      ...
      if (count > LABEL_CUTOFF_SCORE) {
        context.write(new Text(urlTermPair[0]), 
    ...
    public void setup(Context context) {
      try {
        if (URL_DOCID_MAP == null) {
          ...
          NUTCH_INDEX_READER = IndexReader.open(
            FSDirectory.open(new File(
            context.getConfiguration().get("nutch.index.dir"))), true);
   ...
   * index.
   */
  private static class Reducer3 extends 
      Reducer<Text,NutchDocument,Text,NutchDocument> {
    ...
    public void setup(Context context) {
      if (INDEX2_WRITER == null) {
        String indexOutputDir = 
          context.getConfiguration().get("index2.output.dir");
        try {
        INDEX2_WRITER = new IndexWriter(
          FSDirectory.open(new File(indexOutputDir)), 
    ...
    public void reduce(Text key, Iterable<NutchDocument> values, 
      ...
      for (Iterator<NutchDocument> it = values.iterator(); it.hasNext(); ) {
        ...
        doc.add(new Field("label", nutchDoc.getFieldValue("label"), 
          Store.YES, Index.NOT_ANALYZED));
        doc.add(new Field("label_count", nutchDoc.getFieldValue("label_count"), 
          Store.YES, Index.NOT_ANALYZED));
  ...
  private void analyze(Configuration conf, Path indexDir, String dictFile, 
      List<Path> segments,int titleBoost) throws Exception {
  ...
  private void flatten(Configuration conf, Path indexDir, 
      int labelCutoffScore) throws Exception {
    LOGGER.info("Stage 2 (flatten)");
    Job job2 = new Job(conf, "index2-normalize");
    job2.getConfiguration().set(
      "label.cutoff.score", String.valueOf(labelCutoffScore));
  ...
  private void merge(Configuration conf, Path indexDir, 
      String nutchIndexDir) throws Exception {
    LOGGER.info("Stage 3 (merge)");
    Job job3 = new Job(conf, "index2-merge");
    job3.getConfiguration().set("nutch.index.dir", nutchIndexDir);
    job3.getConfiguration().set("index2.output.dir", 
